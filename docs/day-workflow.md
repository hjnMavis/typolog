...
Day N 시작
 ...
 │
 5. 커밋 & PR         세 게이트를 모두 통과했을 때만 커밋·PR. 작업 단위별 PR + PR 내 세분 커밋. (AI 서명 없음)
```
...
### 5. 커밋 & PR
- 세 게이트 통과 후, 그날 작업을 **작업 단위별 PR**로 나눠 연다 (PR 범위 최소화). 단위 간 의존 순서대로 **순차 머지**한다.
- 각 PR 내부는 **논리적 단계별 세분 커밋**으로 구성한다 (예: 설정/패키지 → 구현 → 문서 동기화 → 리뷰 반영).
- 작업 단위(=PR)를 쪼개는 기준은 `docs/agent-view-workflow.md`의 "작업 단위 쪼개는 기준"을 따른다. 분리 금지 케이스(예: DB 스키마 DDL + RLS 정책)는 **PR(머지 단위) 기준**으로 적용한다 — 같은 PR 안에서 커밋을 나누는 것은 무방.
...
| 커밋 / PR | 작업 단위별 PR(순차 머지) + PR 내 세분 커밋 | 메인 세션 | AI 서명 없음 |
